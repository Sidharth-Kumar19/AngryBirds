...
//        done = -1;
    ...
    private void loadEntitiesFromMap() {
        for (MapObject object : map.getLayers().get("Entities").getObjects()) {
            if (object instanceof RectangleMapObject) {
                ...
                switch(type){
                    case "redBird":
                        birds.add(new RedBird(world,new Vector2(rect.x, rect.y),16));
                        break;
                    case "blackBird":
                        birds.add(new BlackBird(world,new Vector2(rect.x, rect.y),16));
                        break;
                    case "yellowBird":
                        birds.add(new YellowBird(world,new Vector2(rect.x, rect.y),16));
                        break;
                    case "minionPig":
    ...
    public void render(float delta){
        ...
        if(Gdx.input.isTouched()){
            //System.out.println("Touched me");
            Vector2 touchpoint = new Vector2(Gdx.input.getX(),Gdx.graphics.getHeight()-Gdx.input.getY());
            Rectangle dragbox = new Rectangle(slingAnchor.x,slingAnchor.y,64,64);
            ...
            if(isDragging){
                ...
                Vector2 launchForce = slingAnchor.cpy().sub(releasePosition).scl(2f);
                //Vector2 launchVelocity = this.currentBird.getBody().getLinearVelocity().scl(0.5f); // Adjust multiplier
                this.currentBird.applyForce(launchForce);
